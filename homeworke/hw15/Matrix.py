...
class Matrix:
    ...
    @init_log
    def __init__(self, matrix: list = None, width: int = 5, height: int = None, filling: FILLING = FILLING_NONE):
        ...
        match filling:
            ...
            case _:
                raise FillingException(filling, FILLING)
    ...
    def _rand(self, min_value=0, max_value=99) -> int:
        """
        Метод для заполнения матрицы
    ...
    def __eq__(self, other) -> bool:
        ...
        """
        if not self & other:
            raise ConsistencyException('проверки на равенство ')
        for i in range(self.width):
            for j in range(self.height):
    ...
    def __add__(self, other):
        if not self & other:
            raise ConsistencyException('складывания ')
...
if __name__ == '__main__':
    print(m := Matrix(filling=FILLING_COUNTER))
    print(n := Matrix(filling=FILLING_COUNTER))
    print(mn := (m * n))
    print(mn == n)
